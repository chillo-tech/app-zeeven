...
import { INFORMATIONS, getInputFormattedDate } from 'utils';
...
function Messages() { 
  ...
  const {state: {stepIndex, campain: {messages = [{text:'', date: '', time: ''}]}}, updateCampain, previousStep} = context;
        ...
        <form noValidate className="block space-y-6" onSubmit={handleSubmit(onSubmit)}>
          {fields.map((field, index) => (
              ...
              <div key={`message-${field.id}`}>
                <label htmlFor="message" className="w-full flex flex-col justify-between mb-2 font-light">
                ...
                <div className="flex justify-between items-center flex-col md:flex-row">
                  ...
                  <div className="flex buttons items-center">
                    <button type='button' className='text-lg' onClick={() => updateStyle({type: 'BOLD'})}>
                      <BiBold className='p-0' size={25}/>
                    </button>
                    <button type='button' className='text-lg px-3' onClick={() => updateStyle({type: 'ITALIC'})}>
                          ...
                          <ul className='informations absolute bottom-0 left-0 right-0 pb-10 bg-white z-10 transition duration-1000'>
                            {
                              INFORMATIONS.map(({label, value}, index) => (
                                <li className='information' key={`information-${index}`}>
                                  <button type="button" className='w-full text-left px-3 py-1 border-b border-slate-200' onClick={() => updateStyle({type: 'VARIABLE', value})}>
                                    {label}
                                  </button>
                                </li>
                              ))
                ...
                <div className="block">
                    <label htmlFor="date" className="w-full flex flex-col justify-between mb-2 font-light">
                  ...
                  <div className="block">
                    <label htmlFor="time" className="w-full flex flex-col justify-between mb-2 font-light">
