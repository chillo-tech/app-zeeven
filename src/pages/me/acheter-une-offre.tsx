...
import { TARIF_FIELDS, formattedNumber, frequenceLabel } from '@/utils';
...
function AcheterUneOffre({ option, product }: { option: string; product: string }) {
  ...
  const tva = (value: string) => {
    return Number(value.replaceAll(' ', '')) * 0.2;
  };
  const total = (value: string) => {
    return Number(tva(value.replaceAll(' ', ''))) + Number(value.replaceAll(' ', ''));
  };
  ...
  const schema = yup
    .object({
      ...
      channel: yup.string().trim().required("Le canal n'est pas défini"),
      credits: yup.string().trim().required("Le nombre de crédits n'est pas défini"),
      frequence: yup.string().trim()
    })
    .required();
  const {
    register,
  ...
  const mutation = useMutation({
    mutationFn: (applicationMessage: Offer) =>
      add('/api/backend/payment?redirect=true', applicationMessage),
    onError: (error: any) => {
                      ...
                      .map(({ pricing_id }: any, index: number) => (
                        <div
                          ...
                          className={classNames(
                            ...
                            'relative rounded-2xl rounded-xl bg-white px-6 py-8 shadow-lg md:px-10',
                        ...
                        >
                          ...
                          <h1 className="text-font-extrabold mb-10 flex flex-col text-black">
                            ...
                            <span className="text-5xl font-extrabold text-black">
                              {formattedNumber(`${total(pricing_id?.value)}`)}&nbsp;
                              {pricing_id?.unit}{' '}
                              {pricing_id?.frequence ? (
                                <span className="text-sm font-normal">
                                  /{frequenceLabel(pricing_id?.frequence)}
                                </span>
                              ) : null}
                            </span>
                          </h1>
                          <p className="flex justify-between text-lg">
                            ...
                            <span className="text-black opacity-70">
                              {formattedNumber(`${pricing_id?.value}`)} {pricing_id?.unit}
                            </span>
                          </p>
                          ...
                          <p className="my-2 flex justify-between border-b border-t border-gray-200 py-2 text-lg">
                            ...
                            <span className="text-black opacity-70">
                              {formattedNumber(`${tva(pricing_id?.value)}`)}&nbsp;{pricing_id?.unit}
                            </span>
                          </p>
                          <p className="flex justify-between text-lg">
                          ...
                          >
                            <div className="block hidden py-3">
                              <input type="text" {...register('productId', { value: data.id })} />
                              <input
                                type="text"
                              ...
                              <input
                                type="text"
                                {...register('amountHT', {
                                  value: pricing_id.value.replaceAll(' ', ''),
                                })}
                              />
                              <input
                                type="text"
                              ...
                              <input
                                type="text"
                                {...register('channel', { value: pricing_id.channel })}
                              />
                              <input
                                type="text"
                                {...register('credits', { value: pricing_id.credits })}
                              />
                              <input
                                type="text"
                                {...register('frequence', { value: pricing_id.frequence })}
                              />
                            </div>
                            <div>
