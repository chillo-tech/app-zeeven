...
const schema = yup
  .object({
    ...
    template: yup.object({
      ...
      file: yup.string().nullable().required('Merci de transmettre un fichier'),
...
function Invitation({ id, slug }: { id: number; slug: String }) {
    ...
    mutationFn: (item: any) =>
      add(
        ...
        JSON.stringify({
          ...
          template: {
            ...item.template,
            schedules: item.template.schedules.map((schedule: any) => ({
              ...schedule,
      ...
      fileReader.onload = () => {
        ...
        if(fileReader.result) {
          ...
          image.onload = function() {
              setValue('template.width', image.width);
              setValue('template.height', image.height);
          };
  ...
  return (
    <ProtectedLayout>
      <Metadata
        entry={{ title: 'Tickets pour évènements', description: 'Tickets pour évènements' }}
      />
      <h1 className="text-3xl font-semibold text-app-blue">
        Invitations pour votre évènement {data?.name}
      </h1>
      {mutation.isLoading ? (
        <Message
          type="loading"
            ...
            {({ open }) => (
              <>
                <Disclosure.Button className="flex w-full justify-between rounded-t-md bg-slate-100 px-4 py-2 text-left font-light text-app-blue focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
                  <span className="flex w-full flex-col justify-between text-lg font-light font-semibold text-app-blue lg:text-xl">
                    Template
                  </span>
                  <BiChevronUp
                    className={`text-xl ${
                      open ? 'rotate-180 transform' : ''
                    } h-5 w-5 text-app-blue`}
                  />
                </Disclosure.Button>
                <Disclosure.Panel className="rounded-b-md bg-slate-100 pb-6 text-gray-500 md:px-32 ">
                  {/* 
                ...
                */}
                  ...
                  <div>
                    {watchAllFields?.template?.file ? (
                      <>
                        ...
                        <div className={classNames({})}>
                          <ImageDisplay
                            base64={true}
                            local={false}
                            image={{ path: watchAllFields?.template?.file, title: 'Template' }}
                      ...
                      <label
                        htmlFor="template"
                        className="flex cursor-pointer flex-col items-center rounded-lg border-2 border-dashed border-indigo-600 py-10 hover:bg-white"
                      >
                        <AiOutlineCloudUpload className="text-7xl text-app-blue" />
                        <span className="text-xs">Votre fichier doit</span>
                        <span className="text-xs">avoir pour dimensions 2000 x 647</span>
                        <span className="text-xs">être au format JPG ou PNG</span>
                        <input
                          className="hidden"
                          id="template"
                          type="file"
                                  ...
                                  .map((schedule: Schedule, index: number) => (
                                    <li key={`schedule-${index}`}>
                                      <span className="outline-blue-button">
                                        {getHumanDate(
                                          getDateWithTime(schedule.date, schedule.time)
                                        )}
                                        <button
                                          type="button"
