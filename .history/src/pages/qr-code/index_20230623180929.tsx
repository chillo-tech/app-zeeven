                ...
                <li key={slugify(item.label)}>
                  <button
                    ...
                    className={classNames(
                      'text-md flex items-center justify-between py-1 pr-5 font-light text-black',
                      'cursor-pointer',
                      { 'border-b-2 border-blue-700': type === item.value }
                    )}
                    onClick={() => setType(item.value)}
                  >
                    {item.value === 'LINK' ? <TfiWorld className="mr-3" /> : null}
                    {item.value === 'WIFI' ? <TfiWorld className="mr-3" /> : null}
                    {item.label}
                  </button>
                </li>
              ))}
