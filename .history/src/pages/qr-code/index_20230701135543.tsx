              ...
              {article_id.articles && article_id.articles.length ? (
                <section className="flex flex-col md:flex-row gap-4 py-4">
                  {article_id.articles?.map(({ related_article_id }: any) => (
                    <div
                      className="flex flex-col justify-arround border border-app-light-blue bg-white px-10 pb-10 pt-5 md:w-11/12"
                      key={`article-${data.id}-${related_article_id.id}`}
                    >
                      <h3 className="mt-4 px-5 text-lg font-extrabold text-black md:px-0 md:text-3xl mb-4">
