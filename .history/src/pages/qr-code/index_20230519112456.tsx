    ...
    <OpenedLayout>
      ...
      <section className='bg-white rounded-lg mb-10 border-2 border-blue-300 container'>
        <div className="options p-5">
          <ul className='flex justify-center gap-6 p-4 text-xl border-b border-gray-400'> 
              {QR_CODES_TYPES.map((item: any) => (
                <li  key={slugify(item.label)}>
                  <label htmlFor={slugify(item.label)}
                        className="flex items-center justify-between shadow-lg px-5 text-black bg-white font-light rounded-md">
                        <input type="radio"
                              className='hidden'
                              value={item.value}
