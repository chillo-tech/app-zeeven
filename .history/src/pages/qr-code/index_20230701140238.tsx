      ...
      <section className="mx-auto mb-10 bg-app-light-gray md:p-0">
        <div className="container">
          ...
          {data?.articles?.map(({ article_id }: any) => (
            <div
              className="container mx-auto rounded-xl border border-app-light-blue bg-white px-10 pb-10 pt-5 shadow-md md:w-11/12"
            ...
            >
              ...
              {article_id.articles && article_id.articles.length ? (
                <section className="flex flex-col gap-4 py-4 md:flex-row">
                  {article_id.articles?.map(({ related_article_id }: any) => (
                    <div
                      className="relative justify-arround flex flex-col border border-app-light-blue bg-white px-10 pb-10 pt-5 md:w-11/12"
                      key={`article-${data.id}-${related_article_id.id}`}
                    >
                      <h3 className="mb-4 mt-4 px-5 text-lg font-extrabold text-black md:px-0 md:text-3xl">
                        <span className="text-app-blue">{related_article_id.label}</span>
                      </h3>
                      <p>{related_article_id.sublabel}</p>
                      <RenderHtmlContent content={related_article_id?.description} />
                      <div className="absolute h-32 w-32 right-0 bottom-0">
                        <ImageDisplay
                          wrapperClasses="h-full border-app-blue relative overflow-hidden shadow-lg"
              ...
              <Link className="text-app-blue underline" href="/contactez-nous">
                Contactez nous
              </Link>
            </div>
          ))}
