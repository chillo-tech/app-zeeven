                ...
                <Tab.Panel key={`tarifs-values-${data.id}-${related_page_id.id}`}>
                  <div className="grid grid-cols-3 gap-4">
                    {related_page_id?.prices.map(({ pricing_id }: any) => (
                      ...
                      >
                        <h3 className="text-center text-xl font-semibold uppercase text-gray-600 font-dark">
                          {pricing_id?.label}
                        </h3>
                        <p className="text-center opacity-75 text-sm text-gray-600  h-6">
                          {pricing_id?.sublabel}
                        </p>
                        <h2 className="font-extrabold text-center text-3xl text-black mt-8">
                          {pricing_id?.value} {pricing_id?.unit}
                        </h2>
                        <p className="text-center opacity-75 text-sm text-gray-600">
                            {pricing_id?.description}
                        </p>
                        <ul>
                          {pricing_id?.items.map(({item_id}: any) => (
                            <li key={`tarifs-values-${related_page_id.id}-${pricing_id.id}-${item_id.id}`}> {item_id?.label}</li>
                          ))}
                        </ul>
