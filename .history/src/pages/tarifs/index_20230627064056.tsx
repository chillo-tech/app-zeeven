  ...
  return (
    <OpenedLayout>
      ...
      <section className="container mx-auto md:w-11/12 pb-20 pt-10">
        {data ? (
          <Tab.Group>
            <Tab.List className="flex space-x-1 rounded-xl bg-app-blue/20 p-1">
              {data?.pages.map(({ related_page_id }: any) => (
                <Tab
                  key={`tarifs-${data.id}-${related_page_id.id}`}
                  className={({ selected }) =>
                    classNames(
                      'text-md w-full rounded-lg py-2 md:py-4 font-medium leading-5 text-app-blue',
            ...
            <Tab.Panels className="mt-8">
              {data?.pages.map(({ related_page_id }: any) => (
                <Tab.Panel key={`tarifs-values-${data.id}-${related_page_id.id}`}>
                  <h1 className="px-5 md:px-0 mt-4 mb-8 text-center text-3xl md:text-4xl font-extrabold text-black">
                    Notre offre de <span className='text-app-blue uppercase'>{related_page_id.label}</span>
                  </h1>
                  <div className="grid md:grid-cols-3 gap-4">
                    {related_page_id?.prices.map(({ pricing_id }: any) => (
                      ...
                      >
                        {pricing_id?.popular ? (
                          <span className='text-white bg-app-blue rounded-full absolute -translate-y-1/2 translate-x-3/4 font-semibold px-8 py-1 text-sm top-0'>
                            Populaire
                          </span>
                        ): null}
                        ...
                        <h2 className="mt-12 text-center text-6xl font-extrabold text-black">
                          {pricing_id?.value} {pricing_id?.unit} {pricing_id?.reference ? (<span className='text-sm font-normal'>/{pricing_id?.reference}</span>) : null}
                        </h2>
                          <RenderHtmlContent content={pricing_id?.description} classes="text-center text-md text-gray-600 opacity-75 mb-4"/>
                        ...
                        <ul className='text-md mb-20'>
                          {pricing_id?.items.map(({ item_id }: any) => (
                            ...
                            >
                              <AiOutlineCheckCircle className='mr-2 text-app-blue'/>{item_id?.label}
                            </li>
                          ))}
                        </ul>
                      </div>
