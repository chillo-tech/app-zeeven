            ...
            <Tab.List className="flex space-x-1 rounded-xl bg-blue-900/20 p-1">
              {data?.pages.map(({ related_page_id }: any) => (
                <Tab
                  ...
                  className={({ selected }) =>
                    classNames(
                      'text-md w-full rounded-lg py-2 md:py-4 font-medium leading-5 text-blue-700',
              ...
              {data?.pages.map(({ related_page_id }: any) => (
                <Tab.Panel key={`tarifs-values-${data.id}-${related_page_id.id}`}>
                  <h1 className="px-5 md:px-0 mt-4 mb-8 text-center text-3xl md:text-4xl font-extrabold text-black">
                    Notre offre de <span className='text-blue-800 uppercase'>{related_page_id.label}</span>
                  </h1>
