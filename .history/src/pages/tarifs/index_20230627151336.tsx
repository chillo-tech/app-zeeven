...
function Tarifs() {
  ...
  return (
    <OpenedLayout>
      ...
      <section className="container mx-auto pb-20 pt-10 md:w-11/12">
        {data ? (
          <Tab.Group>
            ...
            <Tab.Panels className="mt-8">
              {data?.pages.map(({ related_page_id }: any) => (
                <Tab.Panel key={`tarifs-values-${data.id}-${related_page_id.id}`}>
                  ...
                  <div className="bg-app-light-gray py-8 mt-6 rounded-xl text-center text-xl">
                  Besoin d&apos;aide pour choisir une offre? Contactez nos experts QR Code au +1 (646) 655 0702
                  </div>
                </Tab.Panel>
              ))}
            </Tab.Panels>
