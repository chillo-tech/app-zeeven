...
const getFormattedNumber = (entry: string) => {
  return entry.trim().length() > 0 ? entry.getHours() : '0'+entry.getHours()
}
export const getHumanDate = (entry: Date) => {
	...
	return `${dayOfWeekLabel}, ${dayOfDate} ${monthLabel} ${entry.getHours() > 0 ? entry.getHours() : '0'+entry.getHours()}:${entry.getMinutes() > 0 ? entry.getMinutes() : '0'+entry.getMinutes()}`;
