                    ...
                    {company && company.categories && company.categories[0].pages ? (
                      <ul className='flex items-center justify-center gap-6 py-4'> 
                        {company.categories[index].pages.map((page: any) => (
                          <li  key={slugify(`${page.page_id.id}-${page.page_id.label}`)} className='bg-white'>
                            <Link href={slugify(`${page.page_id.id}-${page.page_id.label}`)} className="text-black px-10 font-medium rounded-lg hover:text-white">
                              {page.page_id.label}
                            </Link>
                          </li>
                        ))}
