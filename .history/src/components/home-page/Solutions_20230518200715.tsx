                        ...
                        {company.categories[index].pages.map((page: any) => (
                          <li  key={slugify(`${page.page_id.id}-${page.page_id.label}`)}>
                            <Link href={slugify(`${page.page_id.id}-${page.page_id.label}`)} 
                                  className="flex items-center justify-between shadow-lg py-3 text-black px-10 bg-white font-medium rounded-md hover:text-white"
                            >
                                <ImageDisplay
                                  wrapperClasses = 'relative w-12 h-12'
