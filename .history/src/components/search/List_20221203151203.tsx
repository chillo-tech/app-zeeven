...
function List({data = []}: {data: any[]}) {
  return (
    <div className='search-result h-min-100'>
        ...
        {data.map((item, index)=> (
          <>
          <div className={`grid grid-cols-7 items-center ${index%2 === 1 ? 'bg-blue-50': ''}`} key={item.slug}>
              ...
              <span className='py-3 px-5 flex items-center'>
                {item.channels.map((channelItem: any, channelIndex: number) => {
                  if(channelItem === "EMAIL")  return <CiMail className="ml-2 text-blue-600" size={22} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "WHATSAPP")  return <BsWhatsapp className="ml-2 text-green-600" size={22} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "SMS")  return <FaSms className="ml-2" size={22} key={`channelIndex-${channelIndex}`}/>;
                })}
              </span>
