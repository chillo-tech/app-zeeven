        ...
        {data.map((item, index)=> (
          ...
          <div className="grid grid-cols-7 items-center" key={item.slug}>
              ...
              <span className='py-3 px-5 flex'>
                {item.channels.map((channelItem: any, channelInex: number) => {
                  if(channelItem === "EMAIL")  return <CiMail key={`channelInex-${channelInex}`}/>;
                  if(channelItem === "WHATSAPP")  return <BsWhatsapp key={`channelInex-${channelInex}`}/>;
                  if(channelItem === "SMS")  return <FaSms key={`channelInex-${channelInex}`}/>;
                })}
              </span>
