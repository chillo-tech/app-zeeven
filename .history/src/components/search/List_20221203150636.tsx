          ...
          <div className="grid grid-cols-7 items-center" key={item.slug}>
              ...
              <span className='py-3 px-5 flex'>
                {item.channels.map((channelItem: any, channelIndex: number) => {
                  if(channelItem === "EMAIL")  return <CiMail key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "WHATSAPP")  return <BsWhatsapp key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "SMS")  return <FaSms key={`channelIndex-${channelIndex}`}/>;
                })}
              </span>
