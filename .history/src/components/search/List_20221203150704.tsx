          ...
          <div className="grid grid-cols-7 items-center" key={item.slug}>
              ...
              <span className='py-3 px-5 flex'>
                {item.channels.map((channelItem: any, channelIndex: number) => {
                  if(channelItem === "EMAIL")  return <CiMail size={28} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "WHATSAPP")  return <BsWhatsapp size={28} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "SMS")  return <FaSms size={28} key={`channelIndex-${channelIndex}`}/>;
                })}
              </span>
