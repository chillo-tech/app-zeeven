          ...
          <div className="grid grid-cols-7 items-center" key={item.slug}>
              ...
              <span className='py-3 px-5 flex'>
                {item.channels.map((channelItem: any, channelIndex: number) => {
                  if(channelItem === "EMAIL")  return <CiMail className="ml-2" size={20} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "WHATSAPP")  return <BsWhatsapp className="ml-2 text-green-600" size={20} key={`channelIndex-${channelIndex}`}/>;
                  if(channelItem === "SMS")  return <FaSms className="ml-2 text-blue-600" size={20} key={`channelIndex-${channelIndex}`}/>;
                })}
              </span>
