...
function ScheduleEdit({ update, index, value, control, displayForm }: any) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 tems-end">
        <div className="mb-2 block">
          ...
          />
          {errors.date?.type === 'required' && <p role="text-red-500">Ce champ est requis</p>}
        </div>
        <div className="block">
          <label
            htmlFor="time"
          ...
          />
          {errors.time?.type === 'required' && <p role="text-red-500">Ce champ est requis</p>}
        </div>
        <div className="block">
          <button
