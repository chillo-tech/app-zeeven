...
function Schedules() {
	...
	const {state: {stepIndex, campain: {messages}}, updateCampain, previousStep} = context;
    ...
    setSchedules((current: Schedule[] = []) => {
      const uniqueAuthors = [...current, schedule].reduce(
        (accumulator: Schedule[], item: Schedule) => {
          if (!accumulator.find((entry: Schedule) => entry.date.getTime() === item.date.getTime())) {
                ...
                <li key={`schedule-${index}`}>
                  <span className="outline-blue-button">
                    ...
                    <button type="button" onClick={() => removeSchedule(schedule)}>
                      <RxCrossCircled className="ml-3 text-red-300" />
                    </button>
                  </span>
                </li>
              ))}
