...
function BottomBar({
  ...
  return (
    <div className='flex items-center justify-between mt-6 font-extralight'>
      ...
      {
        nextButtonType  === 'submit'? (
          <button type='submit' className='rounded-md bg-yellow-500 text-app-blue border-yellow-500 px-4 py-2' disabled={nextDisabled}>
            <span>{nextLabel}</span>
          </button>
        ): (
          <button type='button'
            onClick={handleOnClick}
            className='rounded-md bg-yellow-500 text-app-blue border-yellow-500 px-4 py-2' disabled={nextDisabled}>
            <span>{nextLabel}</span>
          </button>
        )
      }
