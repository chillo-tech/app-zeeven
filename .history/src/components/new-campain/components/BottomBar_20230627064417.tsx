...
function BottomBar({
  ...
  return (
    <div className='flex items-center justify-between mt-6 font-extralight'>
      ...
      {
        nextButtonType  === 'submit'? (
          <button type='submit' className='rounded-md bg-text-yellow text-app-blue border-text-yellow px-4 py-2' disabled={nextDisabled}>
            <span>{nextLabel}</span>
          </button>
        ): (
          <button type='button'
            onClick={handleOnClick}
            className='rounded-md bg-text-yellow text-app-blue border-text-yellow px-4 py-2' disabled={nextDisabled}>
            <span>{nextLabel}</span>
          </button>
        )
      }
