    ...
    <div className="py-3">
      ...
      {mutation.isIdle ? (
        <form noValidate className="block space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="block mb-2 flex items-center">
            <label htmlFor="name" className="input-label w-36 flex-none">Nom</label>
          ...
          <div className="block mb-2 flex items-center">
            <label htmlFor="password" className="w-36 flex-none input-label">Mot de passe</label>
      ...
      {[{label: 'WPA 2', value:'WPA2'}].map((plan) => (
        /* Use the `active` state to conditionally style the active option. */
        /* Use the `checked` state to conditionally style the checked option. */
        <RadioGroup.Option key={plan.value} value={plan.value} as={Fragment}>
          {({ active, checked }) => (
            <li
              className={`${
