...
import { WIFI_ENCODAGE, isValidUrl } from '@/utils';
        ...
        <form noValidate className="block space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-2 block flex items-center">
            ...
            <div className="mx-auto flex w-full" aria-labelledby="Encodage">
              <RadioGroup value={plan} onChange={setPlan} name="encodage">
                <div className="flex w-full rounded-md bg-white gap-2">
                  {WIFI_ENCODAGE.map(({ label, value }: { label: string; value: string }) => (
                    <RadioGroup.Option
                      ...
                      value={value}
                      className={({ checked }) => `
                      ${checked ? 'border-indigo-200 bg-blue-900 text-white' : 'border-blue-900 bg-white'}
                      relative flex items-center justitfy-centercursor-pointer rounded-lg px-6 py-2 border
                    `}
                    >
                      {label}
                    </RadioGroup.Option>
